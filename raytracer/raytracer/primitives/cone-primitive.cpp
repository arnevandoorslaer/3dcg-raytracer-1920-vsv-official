...
namespace
{
	...
	public:
		ConeAlongYImplementation() = default;
	...
	protected:
		std::vector<std::shared_ptr<Hit>> find_all_hits(const Ray& ray) const override
		{
			...
			const auto a = -pow(ray.direction.y(), 2) + pow(ray.direction.x(), 2) + pow(ray.direction.z(), 2);
			const auto b = -2 * ray.origin.y() * ray.direction.y() + 2 * ray.origin.x() * ray.direction.x() + 2 * ray.origin.z() * ray.direction.z();
			const auto c = -pow(ray.origin.y(), 2) + pow(ray.origin.x(), 2) + pow(ray.origin.z(), 2);
			const auto D = pow(b, 2) - (4 * a * c);
	...
	public:
		ConeAlongZImplementation() = default;
	...
	protected:
		std::vector<std::shared_ptr<Hit>> find_all_hits(const Ray& ray) const override
		{
			...
			const auto a = -pow(ray.direction.z(), 2) 
				+ pow(ray.direction.y(), 2) 
				+ pow(ray.direction.x(), 2);
			const auto b = -2 * ray.origin.z() * ray.direction.z() + 2 
				* ray.origin.y() * ray.direction.y() + 2 
				* ray.origin.x() * ray.direction.x();
			const auto c = -pow(ray.origin.z(), 2) + pow(ray.origin.y(), 2) 
				+ pow(ray.origin.x(), 2);
			const auto D = pow(b, 2) - (4 * a * c);
