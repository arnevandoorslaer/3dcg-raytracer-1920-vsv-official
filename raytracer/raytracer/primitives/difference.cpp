...
namespace
{
	class DifferenceImplementation : public primitives::_private_::PrimitiveImplementation
	{
	public:
		DifferenceImplementation(Primitive child1, Primitive child2)
		...
		std::vector<std::shared_ptr<Hit>> find_all_hits(const Ray& ray) const override
		{
			...
			while (i1 + 1 != hits1.end() || i2 + 1 != hits2.end())
			{
				...
				// We're inside the differnce if we're not inside both of the children
				bool inside_difference = inside1 && !inside2;
